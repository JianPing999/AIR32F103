...
uint32_t CSD_Tab[4], CID_Tab[4], RCA = 0;//存储CSD，DID，寄存器和卡相对地址
...
SD_Error BusWidth_Config(uint32_t datawidth)
{
	SD_Error errorstatus = SD_OK;
	if (datawidth == SDIO_BusWide_1b)
	{
		errorstatus = SD_EnableWideBusOperation(SDIO_BusWide_1b);
	}
	else if (datawidth == SDIO_BusWide_4b)
	{
		errorstatus = SD_EnableWideBusOperation(SDIO_BusWide_4b);
	}
	else if(datawidth == SDIO_BusWide_8b)
	{
		errorstatus = SD_EnableWideBusOperation(SDIO_BusWide_8b);
	}
	else
	{
		DEBUG("Bus Width Error\n");
	}
...
SD_Error SD_Init(void)
{
	...
	*/
	...
	SDIO_InitStructure.SDIO_ClockDiv = SDIO_TRANSFER_CLK_DIV+65;
	...
	if (errorstatus == SD_OK)
	{
		errorstatus = BusWidth_Config(SDIO_BusWide_4b);
	}  
	...
	{														 
		...
		else if (SDIO_BusWide_4b == WideMode)//4数据线模式
		{
			...
			if (SD_OK == errorstatus)
			{
				...
				SDIO_InitStructure.SDIO_ClockBypass = SDIO_ClockBypass_Disable;
				SDIO_InitStructure.SDIO_ClockPowerSave = SDIO_ClockPowerSave_Disable;
				SDIO_InitStructure.SDIO_BusWide = SDIO_BusWide_4b;	//这个是设air322的sdio的传输方式 ，切换模式必须从卡和sdio都对应好
...
/**
  ...
  * @retval SD_Error: SD Card Error code.
  */
SD_Error SD_ReadBlock(uint8_t *readbuff, uint32_t ReadAddr, uint16_t BlockSize,uint32_t DataMode)
{
	SD_Error errorstatus = SD_OK;
...
/**
  ...
  * @retval SD_Error: SD Card Error code.
  */
SD_Error SD_WriteBlock(uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize,uint32_t DataMode)
{
	SD_Error errorstatus = SD_OK;
...
static SD_Error CmdResp7Error(void)
{
  ...
  status = SDIO->STA;	//读取SDIO状态寄存器 ，此状态寄air32h32的寄存器
...
u8 SD_ReadDisk(u8*buf,u32 sector,u8 cnt)
{
	...
	if((u32)buf%4!=0)
	{
	 	for(n=0;n<cnt;n++)
		{
		 	sta=SD_ReadBlock(SDIO_DATA_BUFFER,lsector+512*n,512,POLLING_MODE);//单个sector的读操作
			memcpy(buf,SDIO_DATA_BUFFER,512);
			buf+=512;
		} 
	}else
	{
		if(cnt==1)sta=SD_ReadBlock(buf,lsector,512,POLLING_MODE);    	//单个sector的读操作
	...
	if((u32)buf%4!=0)
	{
	 	for(n=0;n<cnt;n++)
		{
			memcpy(SDIO_DATA_BUFFER,buf,512);
		 	sta=SD_WriteBlock(SDIO_DATA_BUFFER,lsector+512*n,512,POLLING_MODE);//单个sector的写操作
			buf+=512;
		} 
	}else
	{
		if(cnt==1)sta=SD_WriteBlock(buf,lsector,512,POLLING_MODE);    	//单个sector的写操作
